 $(window).on('load', function () {
 	function preloader(state){$preloader = $('.preloader');if (state == 1) {$preloader.show();}else if (state == 0) {$preloader.hide();}}preloader(0);
	...
	modelObject = {
	  ...
	  "plate": [
	  	{"type":"plate", "color":"gold-polished", "name":"Латунь глянцевая", "price":"0", "domgradient":"#f3cd52,#6b5718,#fffde9", "state":"checked", "load":"true"},
	  	{"type":"plate", "color":"gold-matt", "name":"Латунь матовая", "price":"0", "domgradient":"#614d0c,#f7f1b8", "state":"", "load":"false"},
	  	{"type":"plate", "color":"silver-polished", "name":"Серебро глянцевое, +300 ₽", "price":"300", "domgradient":"#ececec,#676767,#fbfbfb", "state":"", "load":"false"},
	  	{"type":"plate", "color":"silver-matt", "name":"Серебро матовое, +300 ₽", "price":"300", "domgradient":"#5f5f5f,#ececec", "state":"", "load":"false"}
	  ],
	  "font": [
	 ...
	 function init() {
		...
		function update(layer, payload) {
			...
			if (layer == "leather") {
		    	...
		    	else if (($engraveLength < 1)) {$("#engraving-container").text("Надпись");$val = $("#engraving-container").text();}
		    	$engraving = $val;
			}
		}
